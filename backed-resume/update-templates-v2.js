...
const template1 = {
  templateName: 'Simple Black White',
  ...
  sectionStyles: {
    ...
    education: {
      ...
      items: { spacing: '12px', separator: { type: 'none' } }
    },
    experience: {
      container: { marginBottom: '20px' },
      title: {
      ...
      content: { fontSize: '14px', lineHeight: '1.7' },
      items: { spacing: '14px', separator: { type: 'none' } }
    },
    projects: {
      container: { marginBottom: '20px' },
      title: {
...
// ── 模板2: 蓝色专业 (Two Column) ─────────────────────────────────────────────
const template2 = {
  templateName: 'Blue Professional',
  ...
  sectionStyles: {
    ...
    summary: {
      ...
      title: {
        ...
        color: 'var(--colors-primary)',
        marginBottom: '6px',
        borderBottom: '2px solid var(--colors-border)',
        paddingBottom: '3px'
      },
...
async function main() {
  ...
  for (const { id, name, data } of templates) {
    ...
    console.log(`✅ [${id}] "${name}" updated (${(size / 1024).toFixed(1)} KB)`);
  }
