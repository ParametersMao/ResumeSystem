...
版本：2026-07-13（v1.3 安全与知识分层基线）
状态：v1.2 基础链路已验证；v1.3 按本文档先行实施，未通过本文门禁前不得宣称生产 RAG 可用
...
### 0.4 2026-07-13 线上审计基线

本轮开始时确认以下问题存在，必须修复并纳入自动门禁：

- 唯一知识文档为 `disabled`，Qdrant 虽有 2 个 points，但检索为 0；严格来源模式会阻断全部 RAG 请求。
- 本地知识原文件位于 `/uploads/knowledge/...`，当前静态资源路由允许匿名 HTTP 200 下载。
- LLM API key 当前会随系统配置 JSON 明文写入数据库；读取接口虽遮罩，但静态存储未加密。
- 审计日志敏感键列表未覆盖 `apiKey` 与 `smtpPass`。
- 重建索引先删除旧 points，再写新 points，不具备原子切换能力。
- 上传/索引是同步长请求，缺少任务状态、重试与失败恢复。
- NestJS、Agent 与 Nginx 的文件大小限制不一致。
...
生产环境优先从运行时 secret 注入 API key。若必须通过管理后台更新，落库前必须使用独立的主密钥进行认证加密，数据库只保存带版本的密文；主密钥只能存在于服务器 secret，不得与密文同库。API key 不得写入 Git、脚本默认值、Dockerfile、日志或本文档。系统配置读取接口只返回 `apiKeyConfigured`，不会回传明文密钥。NestJS 仅在发起内部 Agent 请求时短暂解密并透传；Agent 不持久化它。审计脱敏必须覆盖 `apiKey`、`smtpPass`、各类 token/secret/authorization，并使用大小写与命名变体归一化匹配。
...
2026-07-13 复核：代码与容器仍可运行，但标准知识文档已被禁用，管理员检索为 0，strict diagnose 返回 502。因此历史通过记录不能替代每次发布的运行态门禁；发布脚本必须断言 `readyEnabledDocuments >= 1`、`collectionPoints >= 1` 和一条 live RAG E2E 成功。
...
- 私有 JD 必须在向量层强制 owner/resume filter，并提供删除与过期清理。
