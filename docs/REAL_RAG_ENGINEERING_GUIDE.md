...
中断 deploy 的 commit 绑定以 rollout marker、候选 `.env` 的完整 `RELEASE_COMMIT`、上一版 `.env` 和回滚备份 `release-commit.txt` 为准；`deploy/release-manifest.env` 只固定语义版本和 Agent 版本，恢复器不得从它读取不存在的 commit，也不得为了解锁而修改不可变候选目录。恢复状态机测试必须使用与生产一致的 version-only manifest，并在 disposable Linux 容器中进入 CI。
