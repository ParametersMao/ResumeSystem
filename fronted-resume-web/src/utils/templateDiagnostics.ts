 ...
 * 诊断模板
 */
export function diagnoseTemplate(templateData: any): DiagnosticResult {
 ...
 * 检测模板格式
 */
function detectTemplateFormat(templateData: any): 'new' | 'old' | 'unknown' {
 ...
 * 检查必需字段
 */
function checkRequiredFields(templateData: any, result: DiagnosticResult) {
 ...
 * 检查模块配置
 */
function checkModuleConfiguration(templateData: any, result: DiagnosticResult) {
  const format = result.format

  if (format === 'old' && Array.isArray(templateData.layout)) {
    templateData.layout.forEach((module: any, index: number) => {
      // 检查模块类型
      if (!module.type) {
        result.errors.push({
          type: 'INVALID_MODULE',
 ...
 * 检查样式配置
 */
function checkStyleConfiguration(templateData: any, result: DiagnosticResult) {
  const format = result.format
 ...
 * 检查颜色配置
 */
function checkColorConfiguration(templateData: any, result: DiagnosticResult) {
 ...
 * 检查布局配置
 */
function checkLayoutConfiguration(templateData: any, result: DiagnosticResult) {
  const format = result.format
 ...
 * 自动修复模板
 */
export function autoFixTemplate(templateData: any, diagnostics: DiagnosticResult): any {
    ...
    switch (fix.type) {
      ...
      case 'ADD_DEFAULT_CONFIG':
        ...
        if (Array.isArray(fixed.layout)) {
          fixed.layout = fixed.layout.map((module: any) => {
            if (!module.config) {
              const normalizedType = normalizeModuleType(module.type)
              module.config = {
 ...
 * 获取默认布局类型
 */
function getDefaultLayout(moduleType: string): string {
 ...
 * 批量诊断多个模板
 */
export function batchDiagnose(templates: Array<{ id: string, data: any }>): Map<string, DiagnosticResult> {
