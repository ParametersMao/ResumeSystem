 ...
 */
export const MODULE_TYPE_ALIASES = {
    // 个人信息模块的各种别名
    'personal': 'basic',
    'personalInfo': 'basic',
    'personal-info': 'basic',
 ...
 * @returns 标准化后的模块类型
 */
export function normalizeModuleType(type) {
    if (!type)
        return 'custom';
 ...
 * @returns 渲染器组件名称
 */
export function getRendererName(type) {
 ...
 * @returns 是否为基础信息模块
 */
export function isBasicInfoModule(type) {
    const normalizedType = normalizeModuleType(type);
 ...
 * @returns 默认配置对象
 */
export function getDefaultModuleConfig(type) {
    const normalizedType = normalizeModuleType(type);
    const defaults = {
        basic: {
            layout: 'flexible',
 ...
 * @returns 是否需要特殊处理
 */
export function needsSpecialHandling(type) {
    const normalizedType = normalizeModuleType(type);
    // 基础信息模块需要特殊处理
    return normalizedType === 'basic';
}
