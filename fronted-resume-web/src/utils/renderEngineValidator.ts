 ...
 * 验证单个模块类型
 */
export function validateModuleType(type: string, sampleData?: any): ValidationResult {
 ...
 * 验证所有标准模块类型
 */
export function validateAllModuleTypes(): ValidationResult[] {
 ...
 * 测试模板数据
 */
export function validateTemplateData(templateData: any): {
  success: boolean
  errors: string[]
  warnings: string[]
  ...
  const moduleResults: ValidationResult[] = []
