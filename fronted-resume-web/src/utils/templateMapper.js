...
export function mergeSectionsFromTemplate(templateSections, resumeSections) {
    ...
    templateSections.forEach((tplSection, index) => {
        ...
        const sourceItems = existing && Array.isArray(existing.items)
            ? clone(existing.items)
            : Array.isArray(tplSection.items)
                ? clone(tplSection.items)
                : [];
        const mergedData = existing?.data || tplSection.data
            ? {
...
export function applyTemplateToResume(templateSections, resumeSections) {
