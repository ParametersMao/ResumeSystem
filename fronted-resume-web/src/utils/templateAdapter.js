 ...
 */
export function adaptLegacyTemplateData(templateData, layoutType = 'single-column', extraStyles = {}) {
    if (!templateData) {
    ...
    if (Array.isArray(oldLayout) && oldLayout.length > 0) {
        ...
        oldLayout.forEach((moduleConfig) => {
            if (!moduleConfig || !moduleConfig.type)
                return;
            // 标准化模块类型
            ...
            // 处理其他特定样式
            if (config.schoolStyle)
                sectionStyles[normalizedType].schoolStyle = config.schoolStyle;
            if (config.degreeStyle)
                sectionStyles[normalizedType].degreeStyle = config.degreeStyle;
            if (config.dateStyle)
                sectionStyles[normalizedType].dateStyle = config.dateStyle;
            if (config.descriptionStyle)
                sectionStyles[normalizedType].descriptionStyle = config.descriptionStyle;
            if (config.companyStyle)
                sectionStyles[normalizedType].companyStyle = config.companyStyle;
            if (config.positionStyle)
                sectionStyles[normalizedType].positionStyle = config.positionStyle;
            if (config.tagStyle)
                sectionStyles[normalizedType].tagStyle = config.tagStyle;
            if (config.skillBars)
                sectionStyles[normalizedType].skillBars = config.skillBars;
        });
    }
 ...
 * @param layoutType 布局类型
 */
function createDefaultTemplate(layoutType = 'single-column') {
    return {
        theme: {
            colors: {
                primary: '#2f80ed',
 ...
 */
export function adaptLegacyResumeData(resumeData) {
    if (!resumeData) {
...
function normalizeSectionItems(type, items) {
