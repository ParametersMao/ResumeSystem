...
export function resolveTemplateVariant(source, templateData) {
    return resolveTemplatePreset(source, templateData).variant;
}
export function resolveTemplatePreset(source, templateData) {
    const inlineVariant = [
    ...
    const matched = TEMPLATE_PRESETS.find((preset) => preset.keywords.some((keyword) => normalized.includes(keyword.toLowerCase())));
    return matched || DEFAULT_TEMPLATE_PRESET;
}
export function getTemplateVariantLabel(variant) {
    return findPresetByVariant(variant).label;
}
function findPresetByVariant(variant) {
