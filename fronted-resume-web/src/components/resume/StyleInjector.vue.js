...
const generatedCss = computed(() => {
    ...
    if (props.responsive && props.responsive.styles) {
        ...
        // 移动端样式
        if (props.responsive.styles.mobile) {
            css.push(`@media (min-width: ${breakpoints.mobile}) {
        ${generateCSSFromObject(props.responsive.styles.mobile, '  ')}
      }`);
        }
        // 平板样式
...
function generateBaseSectionStyles() {
...
// 为各节模块生成样式
function generateSectionStyles(sectionStyles) {
    const css = [];
...
// 生成特殊标题样式
function generateSpecialTitleStyle(sectionType, special) {
